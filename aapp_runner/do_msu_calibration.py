...
def do_msu_calibration(platform, timestamp, orbit_number, hirs_file, working_directory, a_tovs):
    
    calibration_location = "-c -l"
    if a_tovs == 'TOVS':
        cmd = "msucl {0} -s {1} -d {2:%Y%m%d} -h {2:%M%H} -n {3:05d} {4}".format(calibration_location, timestamp, orbit_number, msu_file)
    return True